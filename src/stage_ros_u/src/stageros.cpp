...
StageNode::SubscribeModels()
{
	...
	// TODO - print the topic names nicely as well
        ...
        //new_robot->is_crashed_pub = n_.advertise<std_msgs::Bool>(mapName(IS_CRASHED, r, static_cast<Stg::Model*>(new_robot->positionmodel)), 10);
...
StageNode::WorldCallback()
{
  if( ! ros::ok() ) {
    ...
    for (size_t r = 0; r < this->robotmodels_.size(); ++r)
    {
        ...
        //odom_msg.stall = robotmodel->positionmodel->Stall();
        bool a = robotmodel->positionmodel->Stalled();
        //robotmodel->is_crashed_pub.publish(a);
